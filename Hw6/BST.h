...
protected:
    ...
    {            
        ...
        //判断当前节点是否是最后一个子节点，输出不同的枝杈    
            << (numofChild < 1 ? "┌───" : "└───")<< t->element<< std::endl;            //判断当前节点是否是最后一个子节点，添加不同的前导输出
        //判断是否有兄弟节点            
     ...
     * @param t 当前节点指针
     */
     BinaryNode *detachMin(BinaryNode *&t)//找出该节点为根的子树的最小值并返回该节点，并改变指针指向
     {
        BinaryNode* temp;
        if (t==nullptr) return nullptr;
        if (t->left!=nullptr)
        {
            t->height=max( height( t->left )-1, height( t->right ) ) + 1;
            temp=detachMin(t->left);
            return temp;
        }
        else{
            temp=t;
    ...
    void remove(const Comparable &x, BinaryNode * &t) {
        ...
        else if ( t->left != nullptr && t->right != nullptr )//两个孩子均存在
        {
            BinaryNode* temp = t;
            t=detachMin(t->right);
            t->left=temp->left;
