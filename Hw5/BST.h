...
private:
     ...
     * @param t 当前节点指针
     */
     BinaryNode *detachMin(BinaryNode *&t,BinaryNode *&pnode)//找出该节点为根的子树的最小值并返回该节点，并改变指针指向
     {
        ...
        BinaryNode* parentnode=pnode;
        while (t->left!=nullptr)
        {
            parentnode=t;
            t=t->left;
        }
        parentnode->left=t->right;
        t->right=nullptr;
        return t;
     }
    ...
    void remove(const Comparable &x, BinaryNode * &t) {
        //由于需要找到被删除节点的父母，因此只能采用循环结构
        BinaryNode* parentnode=t;//记录被删除节点的父母
        BinaryNode* temp=t;
        int choice;//记录被删除节点是其父母的左还是右孩子(对应0,1)
        if (t==nullptr) return;
        ...
        else//若左右子树都不空
        {
            ...
            BinaryNode* snode=detachMin(t->right,t->right);
            snode->left=lnode;
            snode->right=rnode;
            t=snode;
            delete tode;
         ...
         //如果删除节点是二叉树的根节点，那么直接替换即可
        //先处理该节点只有一个孩子或者没有孩子的情况
        if (temp->left==nullptr)
        {
            BinaryNode* tode=temp;
            if (choice==0) parentnode->left=temp->right;
            if (choice==1) parentnode->right=temp->right;
            delete tode;
        }
        else if (temp->right==nullptr)
        {
            BinaryNode* tode=temp;
            if (choice==0) parentnode->left=temp->left;
            if (choice==1) parentnode->right=temp->left;
            delete tode;
        }
        else//若左右子树都不空
        {
            BinaryNode* leftnode=temp->left;
            BinaryNode* rightnode=temp->right;
            BinaryNode* subnode=detachMin(temp->right,temp);//返回替代节点指针并完成删除
            if (choice==0) parentnode->left=subnode;
            if (choice==1) parentnode->right=subnode;//完成父母一端的链接
            subnode->left=leftnode;
